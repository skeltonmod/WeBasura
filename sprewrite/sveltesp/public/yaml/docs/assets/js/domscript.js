	...
	let domscripts = {
		...
		installStickyMenu: function installStickyMenu () {
			...
			if (nav.find('.ym-hlist').length > 0 ) {
				...
				let menu =	'<li><label for="ym-smnav" style="padding: 0 0.5em; color: rgba(255,255,255,.8)">Navigation:</label> '+
				...
				let smnav = $(menu).appendTo(nav.find('ul')).hide();
				...
				$(document).bind('scroll',function(){
					let hOffset = header.offset().top+header.height(),
...
$(window).load(function() {
	let fragment = location.href.split('#'),
		nav      = $('nav#level2');
