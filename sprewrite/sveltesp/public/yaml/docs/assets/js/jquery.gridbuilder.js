...
(function ($) {
  $.fn.gridBuilder = function (useroptions) {
    return this.each(function () {
      ...
      if ( $.fn.gridBuilder.featureDetection() ) {
		  ...
		  let gridCanvas = $.fn.gridBuilder.makeCanvas(options);
  ...
  $.fn.gridBuilder.drawVertical = function (gridContext, options) {
    if (options.horizontal) {
      ...
      for (let x = - (options.gutter/2) - 0.5; x <= options.horizontal + options.gutter; x += options.horizontal) {
      ...
      if (options.secondaryColor) {
        let xs = (options.gutter/2) + (options.horizontal / 2) - 0.5;
        gridContext.beginPath();
  ...
  $.fn.gridBuilder.setBackground = function (element, gridCanvas, options) {
    let canvasData = gridCanvas.toDataURL();
    $(element).css({
